...
// 4. Query Utama untuk Tabel (Join dengan Absensi Hari Ini)
$today = date('Y-m-d');
$sql = "SELECT tb_siswa.*, tb_kelas.nama_kelas, 
        abs.jam_masuk, abs.jam_keluar, abs.status, abs.tanggal as tgl_absen
        FROM tb_siswa 
        LEFT JOIN tb_kelas ON tb_siswa.id_kelas = tb_kelas.id 
        LEFT JOIN tb_absensi abs ON tb_siswa.id = abs.user_id 
            AND abs.role = 'siswa' 
            AND abs.tanggal = :today
        WHERE 1=1";
...
if ($admin_id) {
    // Reuse existing logic variables if possible, but let's be safe and re-query or reuse variables
    // Siswa index already checks for wali kelas logic above ($admin_kelas_id).
    // We can reuse that partially, but let's stick to the consolidated block for the *display string*.
                ...
                <table class="min-w-full leading-normal">
                    <thead>
                        <tr>
                            ...
                            <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Status (Hari Ini)</th>
                            <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Jam Masuk</th>
                            <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Jam Keluar</th>
                        ...
                        <?php if(count($siswa) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                                    <?php 
                                        $status = ucfirst($row['status'] ?? 'Belum Hadir');
                                        $color = 'gray';
                                        ...
                                        $bgClass = "bg-{$color}-100 text-{$color}-800";
                                        if ($status == 'Belum Hadir') $bgClass = "bg-gray-100 text-gray-500";
                                    ?>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center font-mono text-gray-700">
                                    <?= !empty($row['jam_masuk']) ? date('H:i', strtotime($row['jam_masuk'])) : '-' ?>
                                </td>

                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center font-mono text-gray-700">
                                    <?= !empty($row['jam_keluar']) ? date('H:i', strtotime($row['jam_keluar'])) : '-' ?>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        <button onclick="requestAction('edit.php?id=<?= $row['id'] ?>')" class="text-blue-600 hover:text-blue-900 bg-blue-100 px-3 py-1 rounded-md transition hover:bg-blue-200">Edit</button>
                                        <button onclick="requestAction('delete.php?id=<?= $row['id'] ?>')" class="text-red-600 hover:text-red-900 bg-red-100 px-3 py-1 rounded-md transition hover:bg-red-200">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
